...
export async function resolvePageSize(notePath, pageNum) {
  ...
  if (notePath) {
    try {
      ...
      log(TAG, `getPageSize p${hostPage} ${pageSize.width}x${pageSize.height}`);
    } catch (e) {
...
export function createElementCache(notePath) {
  ...
  async function loadPage(hostPage) {
    ...
    for (const idx of indices) {
      const res = await withTimeout(
        PluginFileAPI.getElements(idx, notePath),
        15000,
        `getElements(${idx})`,
      );
      if (res?.success && Array.isArray(res.result) && res.result.length > 0) {
        merged = res.result;
        log(TAG, `getElements(${idx}) count=${merged.length}`);
        break;
      }
    }
    cache.set(hostPage, merged);
    return merged;
  }
